...
RSpec.describe IndicatorHub do
  ...
  describe ".sma" do
    it "calculates correct values" do
      result = described_class.sma(sequential, period: 5)
    ...
    it "handles hash data" do
      result = described_class.sma(sequential_hash, period: 5)
      expect(result[4]).to eq(12.0)
    end

    it "handles custom field" do
      data = sequential.map { |v| { open: v.to_f * 2 } }
      result = described_class.sma(data, period: 5, field: :open)
  ...
  describe ".ema" do
    it "calculates correct values" do
      result = described_class.ema(sequential, period: 5)
      expect(result[0..3]).to all(be_nil)
  ...
  describe ".wma" do
    it "calculates correct weighted values" do
      result = described_class.wma(sequential, period: 3)
    ...
    it "weights recent prices more than SMA for uptrend" do
      wma = described_class.wma(sequential, period: 5)
      sma = described_class.sma(sequential, period: 5)
  ...
  describe ".rsi" do
    it "returns values between 0 and 100" do
      result = described_class.rsi(rsi_data, period: 14)
      expect(result.compact).to all(be_between(0, 100))
    end
    ...
    it "returns near 100 for strictly rising data" do
      ...
      result = described_class.rsi(rising, period: 14)
      expect(result.compact.last).to be >= 95.0
    end

    it "handles flat data without crash" do
      expect { described_class.rsi(flat_data, period: 14) }.not_to raise_error
    end
  end

  describe ".bb" do
    it "calculates correct band values" do
      result = described_class.bb(sequential, period: 5, standard_deviations: 2)
      expect(result[0..3]).to all(be_nil)
    ...
    it "has symmetric bands" do
      result = described_class.bb(sequential, period: 5)
      (4..10).each do |i|
        b = result[i]
  ...
  describe ".macd" do
    it "macd = fast_ema - slow_ema" do
      fast = described_class.ema(sequential, period: 3)
      slow = described_class.ema(sequential, period: 5)
      macd = described_class.macd(sequential, fast_period: 3, slow_period: 5, signal_period: 3)
    ...
    it "histogram = macd - signal" do
      result = described_class.macd(sequential, fast_period: 2, slow_period: 5, signal_period: 3)
      result.each do |v|
  ...
  describe ".dlr" do
    it "calculates log return correctly" do
      result = described_class.dlr([100.0, 110.0, 105.0])
      expect(result[0]).to be_nil
  ...
  describe ".dr" do
    it "calculates daily return correctly" do
      result = described_class.dr([100.0, 110.0, 105.0])
      expect(result[0]).to be_nil
  ...
  describe ".roc" do
    it "calculates rate of change correctly" do
      result = described_class.roc([100.0, 110.0, 120.0, 115.0, 105.0, 125.0], period: 3)
  ...
  describe ".cmo" do
    it "calculates correctly" do
      result = described_class.cmo([10.0, 12.0, 11.0, 14.0, 13.0], period: 3)
      expect(result[0..2]).to all(be_nil)
    ...
    it "returns 0 for flat data" do
      result = described_class.cmo(flat_data, period: 5)
      expect(result.compact).to all(eq(0.0))
    end
  end

  describe ".dpo" do
    it "returns correct output size with nil padding" do
      result = described_class.dpo(sequential, period: 5)
  ...
  describe ".trix" do
    it "has nil values due to triple smoothing" do
      result = described_class.trix(sequential, period: 3)
      expect(result.size).to eq(sequential.size)
  ...
  describe ".tsi" do
    it "returns values between -100 and 100" do
      ...
      result = described_class.tsi(oscillating, fast_period: 3, slow_period: 5)
      expect(result.compact).to all(be_between(-100, 100))
    end
  end

  describe ".wilders_smoothing" do
    it "first value is SMA of first period elements" do
      result = described_class.wilders_smoothing(sequential, period: 5)
      expect(result[0..3]).to all(be_nil)
  ...
  describe ".cr" do
    it "calculates cumulative return" do
      result = described_class.cr([100.0, 110.0, 105.0])
  ...
  describe ".envelopes_ema" do
    it "applies percentage bands around EMA" do
      result = described_class.envelopes_ema(sequential, period: 5, percentage: 5)
  ...
  describe ".adi" do
    it "calculates ADI with correct CLV" do
      ...
      result = described_class.adi(data)
    ...
    it "returns 0 when high == low" do
      ...
      expect(described_class.adi(data)[0]).to eq(0.0)
    end
  end
  ...
  describe ".adtv" do
    it "calculates average volume" do
      ...
      result = described_class.adtv(data, period: 3)
  ...
  describe ".adx" do
    it "returns values between 0 and 100 with nil padding" do
      result = described_class.adx(ohlcv_large, period: 14)
  ...
  describe ".ao" do
    it "returns output with nil warm-up" do
      result = described_class.ao(ohlcv_large, short_period: 5, long_period: 10)
      expect(result.size).to eq(100)
  ...
  describe ".atr" do
    it "returns non-negative values" do
      result = described_class.atr(ohlcv_large, period: 14)
      expect(result.size).to eq(100)
      expect(result.compact).to all(be >= 0)
    end
  end

  describe ".cci" do
    it "oscillates around zero for varied data" do
      result = described_class.cci(ohlcv_large, period: 20)
    ...
    it "returns 0 for flat data" do
      result = described_class.cci(flat_ohlcv, period: 5)
      expect(result.compact).to all(eq(0.0))
    end
  end
  ...
  describe ".cmf" do
    it "returns values in valid range" do
      result = described_class.cmf(ohlcv_large, period: 20)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".dc" do
    it "returns bands with upper >= middle >= lower" do
      result = described_class.dc(ohlcv_large, period: 20)
  ...
  describe ".eom" do
    it "returns correct output size" do
      result = described_class.eom(ohlcv_large, period: 14)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".fi" do
    it "returns correct output size" do
      result = described_class.fi(ohlcv_large, period: 13)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".ichimoku" do
    it "returns all five components with nil warm-up" do
      result = described_class.ichimoku(ohlcv_large)
  ...
  describe ".imi" do
    it "returns values between 0 and 100" do
      result = described_class.imi(ohlcv_large, period: 14)
      expect(result.compact).to all(be_between(0, 100))
    end
  end
  ...
  describe ".kc" do
    it "returns upper >= middle >= lower" do
      result = described_class.kc(ohlcv_large, period: 20)
  ...
  describe ".kst" do
    it "returns kst and signal" do
      result = described_class.kst(ohlcv_large)
  ...
  describe ".mfi" do
    it "returns values between 0 and 100" do
      result = described_class.mfi(ohlcv_large, period: 14)
      expect(result.compact).to all(be_between(0, 100))
    end
  end
  ...
  describe ".mi" do
    it "returns correct output size" do
      result = described_class.mi(ohlcv_large, period: 25)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".nvi" do
    it "starts at 1000 and only changes when volume decreases" do
      result = described_class.nvi(ohlcv_varied)
  ...
  describe ".obv" do
    it "calculates OBV with up/down/flat days" do
      ...
      result = described_class.obv(data)
      expect(result[0]).to eq(0.0)
  ...
  describe ".obv_mean" do
    it "returns correct output size" do
      result = described_class.obv_mean(ohlcv_large, period: 10)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".pivot_points" do
    it "calculates correct levels" do
      ...
      pp = described_class.pivot_points(data)[0]
    ...
    it "maintains r3 >= r2 >= r1 >= p >= s1 >= s2 >= s3" do
      described_class.pivot_points(ohlcv_large).each do |pp|
  ...
  describe ".price_channel" do
    it "returns upper >= lower" do
      result = described_class.price_channel(ohlcv_large, period: 20)
  ...
  describe ".qstick" do
    it "returns correct output size" do
      result = described_class.qstick(ohlcv_large, period: 10)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".rmi" do
    it "returns values between 0 and 100" do
      result = described_class.rmi(ohlcv_large, period: 14, momentum_period: 5)
      expect(result.compact).to all(be_between(0, 100))
    end
  end
  ...
  describe ".so" do
    it "returns %K and %D between 0 and 100" do
      result = described_class.so(ohlcv_large, k_period: 14, d_period: 3)
  ...
  describe ".uo" do
    it "returns values between 0 and 100" do
      result = described_class.uo(ohlcv_large)
      expect(result.compact).to all(be_between(0, 100))
    end
  end
  ...
  describe ".vi" do
    it "returns non-negative plus_vi and minus_vi" do
      result = described_class.vi(ohlcv_large, period: 14)
  ...
  describe ".volume_oscillator" do
    it "returns nil before warm-up then values" do
      result = described_class.volume_oscillator(ohlcv_large, short_period: 5, long_period: 20)
  ...
  describe ".vpt" do
    it "returns correct output size" do
      result = described_class.vpt(ohlcv_large)
      expect(result.size).to eq(100)
    end
  end
  ...
  describe ".vwap" do
    it "calculates VWAP correctly" do
      ...
      result = described_class.vwap(data)
  ...
  describe ".wr" do
    it "returns values between -100 and 0" do
      result = described_class.wr(ohlcv_large, period: 14)
      expect(result.compact).to all(be_between(-100, 0))
    end
  end
  ...
  describe "edge cases" do
    it "handles empty data for OHLCV indicators" do
      expect(described_class.obv([])).to eq([])
      expect(described_class.pivot_points([])).to eq([])
    end

    it "handles flat data without errors" do
      expect { described_class.ema(flat_data, period: 5) }.not_to raise_error
      expect { described_class.bb(flat_data, period: 5) }.not_to raise_error
      expect { described_class.rsi(flat_data, period: 14) }.not_to raise_error
      expect { described_class.cci(flat_ohlcv, period: 5) }.not_to raise_error
    end
  end
end
